...
/*********20161212 for gesture begin **************/
#if GTP_GESTURE_WAKEUP
static int gesture_flag=0;
#define   TPD_DRIVER_CLASS_NAME    "GT9110P"
#define   ORATATION_DRIVER_CLASS_NAME    "GT9110P_ORATATION"
static int oratation_flag=0;
...
static ssize_t oratation_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	sprintf(buf, "%d\n",oratation_flag);
	ret = strlen(buf) + 1;
	//GTP_INFO("zch----GT9110P %s on_off=%d\n",__func__,ret);
	return ret;
}
static ssize_t oratation_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int on_off = simple_strtoul(buf, NULL, 10);
	//GTP_INFO("zch----GT9110P %s on_off=%d\n",__func__,on_off);
	oratation_flag = on_off;
	return size;
}

static DEVICE_ATTR(oratation, 0664 , oratation_read, oratation_write);
...
static int gesture_create_sysfs(struct i2c_client *client)
{
	...
	tpd_dev = device_create(tpd_class, NULL, tpd_devno, NULL, "gt9110p");
	if(NULL == tpd_dev)
	{
		return -EIO;
	}
	err = device_create_file(tpd_dev, &dev_attr_gesture);
    	err = device_create_file(tpd_dev, &dev_attr_oratation);

	return err;
}

#endif
/*********20161212 for gesture end **************/
...
void process_oratation_data_for_pen(int*x ,int*y)
{
    int tmp;
    switch(oratation_flag){
    case 90:
        tmp = *x;
        *x=*y;
        *y=GTP_MAX_WIDTH - tmp;
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	//GTP_INFO("key_value:%02x, pre_key:%02x, finger:%02x", key_value, pre_key,finger);
            ...
            if ((id & 0x80)) 
			{
				if(!input_w)
				{
				   GTP_DEBUG("Pen touch DOWN(Slot)!");
					...
					//if(0)
					//{
                        			//process_oratation_data_for_pen(&input_x,&input_y);
 ...
 */
	...
	#if GTP_CHARGER_SWITCH	//FIX charger_cfg is  as group7
	ret = gtp_parse_dt_cfg(&ts->client->dev, &config_charger[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, 7);
	#endif
#else 
...
#if GTP_CHARGER_SWITCH
u32 gtp_get_charger_status(void)
{
...
#if 1//def GTP_CONFIG_OF	/* device tree support */
...
for (count = 0; count < 5; count++)
			{
                ...
                gtp_i2c_write(i2c_connect_client, chr_cmd, 3);
				mdelay(50);
				ret=gtp_i2c_read(i2c_connect_client, chr_state, 3);
				if((chr_state[2]&0x1))
					count = 5;			
                               GTP_DEBUG("chr_state[2] = %d\n",chr_state[2]);
