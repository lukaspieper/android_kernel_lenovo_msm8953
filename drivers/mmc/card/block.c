...
bool special_card = false;
static bool cap2_clk_scale = false;
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	/* wangs: Compatible with Japan SD */
	if(!strcmp(mmc_card_name(card), "SU04G")) {
		printk("%d wangs: <%s> '%s' is Japan SD !\n", __LINE__, __func__, mmc_hostname(card->host));
		cap2_clk_scale = card->host->caps2 & MMC_CAP2_CLK_SCALE;
		if (cap2_clk_scale)
			card->host->caps2 &= ~MMC_CAP2_CLK_SCALE;
		special_card = true;
	} else {
		special_card = false;
	}
