...
#if ADDED_BY_HQ_WWM
#define FUSB_DT_GPIO_USB_ID             "qcom,usb_id"               // Name of the USB_ID GPIO pin in the Device Tree
#define FUSB_DT_GPIO_VCONN_EN           "qcom,vconn_en"             // Name of the VCONN_EN GPIO pin in the Device Tree
#endif
...
#if ADDED_BY_HQ_WWM
struct qpnp_pin_cfg qpnp_pin_gpio8_pdic=
{
...
struct qpnp_pin_cfg qpnp_pin_gpio8_default=
{
	.mode = QPNP_PIN_MODE_DIG_OUT,
...
FSC_S32 fusb_InitializeGPIO(void)
{
    ...
    chip->gpio_UsbId = of_get_named_gpio(node, FUSB_DT_GPIO_USB_ID, 0);
    if (!gpio_is_valid(chip->gpio_UsbId))
    {
        dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for USB_ID! Error code: %d\n", __func__, chip->gpio_UsbId);
        return chip->gpio_UsbId;
    }

    printk(KERN_ERR "LANCELOT qpnp_pin_config\n");
    ret = qpnp_pin_config(chip->gpio_UsbId, &qpnp_pin_gpio8_pdic);
    if(ret < 0)
    {
        dev_err(&chip->client->dev, "%s - Error: Could not qpnp_pin_config for USB_ID! Error code: %d\n", __func__, ret);
        return ret;
	
    }

    // Request our GPIO to reserve it in the system - this should help ensure we have exclusive access (not guaranteed)
    ret = gpio_request(chip->gpio_UsbId, FUSB_DT_GPIO_USB_ID);
    if (ret < 0)
    {
        dev_err(&chip->client->dev, "%s - Error: Could not request GPIO for USB_ID! Error code: %d\n", __func__, ret);
        return ret;
    }

    chip->gpio_UsbId_value = 1;
    ret = gpio_direction_output(chip->gpio_UsbId, chip->gpio_UsbId_value);
    if (ret < 0)
    {
        dev_err(&chip->client->dev, "%s - Error: Could not set GPIO direction to output for USB_ID! Error code: %d\n", __func__, ret);
        return ret;
    }

    /* Get our GPIO pins from the device tree, allocate them, and then set their direction (input/output) */
    chip->gpio_VconnEn = of_get_named_gpio(node, FUSB_DT_GPIO_VCONN_EN, 0);
    if (!gpio_is_valid(chip->gpio_VconnEn))
    {
        dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for VCONN_EN! Error code: %d\n", __func__, chip->gpio_VconnEn);
        return chip->gpio_VconnEn;
    }

    // Request our GPIO to reserve it in the system - this should help ensure we have exclusive access (not guaranteed)
    ret = gpio_request(chip->gpio_VconnEn, FUSB_DT_GPIO_VCONN_EN);
    if (ret < 0)
    {
        dev_err(&chip->client->dev, "%s - Error: Could not request GPIO for VCONN_EN! Error code: %d\n", __func__, ret);
        return ret;
    }

    chip->gpio_VconnEn_value = 0;
    ret = gpio_direction_output(chip->gpio_VconnEn, chip->gpio_VconnEn_value);
    if (ret < 0)
    {
        dev_err(&chip->client->dev, "%s - Error: Could not set GPIO direction to output for VCONN_EN! Error code: %d\n", __func__, ret);
        return ret;
    }

#if 1
    // GPIO must be valid by this point
    if (gpio_cansleep(chip->gpio_VconnEn))
    {
        /*
         ...
         */
        gpio_set_value_cansleep(chip->gpio_VconnEn, 0);
    }
    else
    {
        gpio_set_value(chip->gpio_VconnEn, 0);
    }

    pr_debug("FUSB  %s - USB_ID set to: %d\n", __func__, chip->gpio_VconnEn_value ? 1 : 0);
#endif
...
#if ADDED_BY_HQ_WWM
void fusb_GPIO_Set_UsbId(FSC_BOOL set)
{
    struct fusb30x_chip* chip = fusb30x_GetChip();
    if (!chip)
    {
    ...
    // GPIO must be valid by this point
    if (gpio_cansleep(chip->gpio_UsbId))
    {
        /* 
         * If your system routes GPIO calls through a queue of some kind, then
         * it may need to be able to sleep. If so, this call must be used.
         */
        gpio_set_value_cansleep(chip->gpio_UsbId, set ? 1 : 0);
    }
    else
    {
        gpio_set_value(chip->gpio_UsbId, set ? 1 : 0);
    }
    chip->gpio_UsbId_value = set;

    pr_debug("FUSB  %s - USB_ID set to: %d\n", __func__, chip->gpio_UsbId_value ? 1 : 0);
    ...
    if (gpio_cansleep(chip->gpio_VconnEn))
    {
         ...
         * it may need to be able to sleep. If so, this call must be used.
         */
        gpio_set_value_cansleep(chip->gpio_VconnEn, set ? 0 : 1);
    }
    else
    {
        gpio_set_value(chip->gpio_VconnEn, set ? 0 : 1);
    }
    chip->gpio_VconnEn_value = set;
...
#if ADDED_BY_HQ_WWM
void fusb_GPIO_Cleanup_usbid(void)
{
    struct device_node* node;
    struct fusb30x_chip* chip = fusb30x_GetChip();
    if (!chip)
    ...
    printk(KERN_ERR "revers qpnp_pin_config\n");
    pr_err("FUSB  %s - chip->gpio_UsbId = %d\n", __func__, chip->gpio_UsbId);
    if (gpio_is_valid(chip->gpio_UsbId))
    {
        pr_err("FUSB  %s - chip->gpio_UsbId = %d\n", __func__, chip->gpio_UsbId);
        qpnp_pin_config(chip->gpio_UsbId, &qpnp_pin_gpio8_default);
        gpio_free(chip->gpio_UsbId);
        pr_err("FUSB  %s - chip->gpio_UsbId = %d\n", __func__, chip->gpio_UsbId);
    }
}
#endif
...
static ssize_t _fusb_get_pd_status_show(struct device* dev, struct device_attribute* attr, char* buf)
{
	FSC_U32 count = 0;
	FSC_U8 pdstatus = GetUSBPDStatusOverview();

	count += sprintf(buf +count,"USBPDEnabled:[%d]\n",( pdstatus & 0x01)?1:0);
	count += sprintf(buf +count,"USBPDActive:[%d]\n",(pdstatus & 0x02)?1:0);
	count += sprintf(buf +count,"PolicyIsSource:[%d]\n",(pdstatus & 0x04)?1:0);
	count += sprintf(buf +count,"PolicyIsDFP:[%d]\n",(pdstatus & 0x08)?1:0);
	count += sprintf(buf +count,"PolicyHasContract:[%d]\n",(pdstatus & 0x10)?1:0);
	count += sprintf(buf +count,"USBPDBufOverflow:[%d]\n",(pdstatus & 0x80)?1:0);

	return  count;
}

static DEVICE_ATTR(pd_status,S_IRUSR | S_IRGRP | S_IROTH,_fusb_get_pd_status_show,NULL);
