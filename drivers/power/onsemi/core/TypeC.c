...
void StateMachineTypeC(void)
{
...
#else
	if (Registers.Status.I_GCRCSENT != 0){
		Registers.Status.Interrupt1 = 0;
		Registers.Status.InterruptAdv = 0;
		Registers.Status.I_GCRCSENT = 1;
	}
        else{
		Registers.Status.Interrupt1 = 0;
		Registers.Status.InterruptAdv = 0;
	...
	if (g_Idle){
		DeviceRead(regStatus1, 1, &Registers.Status.byte[5]);
		g_Idle = Registers.Status.RX_EMPTY;
	}
#endif
...
void StateMachineAttachWaitSink(void)
{
    ...
    else if((VCONNTerm >= CCTypeRdUSB) && (VCONNTerm <= CCTypeRd3p0))
    {
        updateVCONNSink();
    }
    debounceCC();
...
{  
    ...
    {
        updateVCONNSource();
        WriteStateLog(&TypeCStateLog, 110, 
                VCONNTerm, 
                CCTermCCDebounce);
	if(VCONNTerm > CCTermCCDebounce)
	{
		SetStateDelayUnattached();
	}
    }
...
#ifdef FSC_HAVE_ACCMODE
    if (audioAccSupport && (CCTermCCDebounce == CCTypeRa) && (VCONNTerm == CCTypeRa)) // If both pins are Ra, it's an audio accessory
    {
        WriteStateLog(&TypeCStateLog, 111, 
                VCONNTerm, 
                CCTermCCDebounce);
    ...
    {
        WriteStateLog(&TypeCStateLog, 112, 
                VCONNTerm, 
                CCTermCCDebounce);
        SetStateDebugAccessorySource();
    }
    else if (((CCTermCCDebounce >= CCTypeRdUSB) && (CCTermCCDebounce < CCTypeUndefined)) && ((VCONNTerm == CCTypeOpen) || (VCONNTerm == CCTypeRa)))      // If CC1 is Rd and CC2 is not...
    {
        WriteStateLog(&TypeCStateLog, 113, 
                VCONNTerm, 
                CCTermCCDebounce);
...
#endif // FSC_HAVE_DRP
            {
                WriteStateLog(&TypeCStateLog, 114, 
                VCONNTerm, 
                CCTermCCDebounce);
        ...
        case 0:
            ...
            if ((CCTermPrevious == CCTypeOpen) && (!IsPRSwap))                       // If the debounced CC pin is detected as open and we aren't in the middle of a PR_Swap
            {
                ...
                {   
                    ...
                    platform_set_timer(&StateTimer, tSafe0V);
//                    Registers.Switches.byte[0] = 0x03;                              // Disabled until vSafe0V
//                    DeviceWrite(regSwitches0, 1, &Registers.Switches.byte[0]);
//                    SetStateDelayUnattached();
                }
...
void StateMachineAudioAccessory(void)
{
    ...
    else if(CCTermPrevious == CCTypeRa)
    {
        Registers.Mask.M_COMP_CHNG = 0;
        DeviceWrite(regMask, 1, &Registers.Mask.byte);
        platform_enable_timer(FALSE);
...
void stateMachineTrySink(void)
{
    switch(TypeCSubState)
    {
        case 0:
            if (platform_check_timer(&StateTimer))
            {
                platform_set_timer(&StateTimer, tDRPTryWait);
                TypeCSubState++;
            }
            break;
        case 1:
...
#ifdef FSC_HAVE_ACCMODE
            if ((platform_check_timer(&StateTimer)) && (PortType == USBTypeC_Sink))
            {
...
void SetStateTryWaitSource(void)
{
    ...
    platform_set_timer(&StateTimer, tDRPTry);                                         // Disable the state timer, not used in this state
}
#endif // FSC_HAVE_DRP
...
void setStateSource(FSC_BOOL vconn)
{
    ...
    platform_set_timer(&PDFilterTimer, T_TIMER_DISABLE);                                            // Disable PD filter timer
    platform_set_timer(&PDDebounceTimer, tPDDebounce);                                              // Once in this state, we are waiting for the lines to be stable for tCCDebounce before changing states
    platform_set_timer(&CCDebounceTimer, tCCDebounce);                                          // Disable the 2nd level debouncing initially to force completion of a 1st level debouncing                                        // Once we are in the audio.accessory state, we are going to stop toggling and only monitor CC1
#ifdef FSC_DEBUG
    WriteStateLog(&TypeCStateLog, ConnState, 
                platform_get_system_time(), 
                0);
...
void setStateSink(void)
{
    ...
    platform_set_timer(&PDDebounceTimer, tPDDebounce);                                // Set the tPDDebounce for validating signals to transition to
    platform_set_timer(&CCDebounceTimer, tCCDebounce);                                     // Disable the 2nd level debouncing until the first level has been debounced                              // Set the toggle timer to look at each pin for tDeviceToggle duration
    platform_set_timer(&PDFilterTimer, T_TIMER_DISABLE);  // Disable PD filter timer
#ifdef FSC_DEBUG
    WriteStateLog(&TypeCStateLog, ConnState, 
                platform_get_system_time(), 
...
void clearState(void)
{
	...
	if(!IsHardReset)
	{
		pr_info("FUSB set PolicyIsDFP=0 in %s \n",__func__);
		platform_set_data_role(0);//jack
		PolicyIsDFP = 0;
	}
...
void SetStateIllegalCable(void)
{
    ...
    Registers.Control.HOST_CUR = 0b10;                              // Must advertise 1.5A current
    DeviceWrite(regControl0, 1, &Registers.Control.byte[0]);
    
    Registers.Measure.MDAC = MDAC_0P840V;
    DeviceWrite(regMeasure, 1, &Registers.Measure.byte);           // Commit the DAC threshold
