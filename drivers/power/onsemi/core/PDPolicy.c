...
extern void fusb_GPIO_Set_UsbId(FSC_BOOL set);
/////////////////////////////////////////////////////////////////////////////
...
void USBPDEnable(FSC_BOOL DeviceUpdate, SourceOrSink TypeCDFP)
{
    ...
    if (USBPDEnabled == TRUE)
    {
        ...
        if (blnCCPinIsCC1 || blnCCPinIsCC2)                                     // If we know what pin the CC signal is...
        {
            ...
            Registers.Power.PWR = 0x7;                                         // Enable the internal oscillator for USB PD
            Registers.Power.PD_RCVR_DIS = 1;
            DeviceWrite(regPower, 1, &Registers.Power.byte);                // Commit the power setting
            
            platform_set_timer(&SwapSourceStartTimer, 0); /* Clear SwapSourceStarttimer */
...
void USBPDDisable(FSC_BOOL DeviceUpdate)
{
    ...
    ProtocolFlushTxFIFO();
        ...
        Registers.Status.I_COMP_CHNG = 1; // Force VBUS check
}
...
void PolicySourceStartup(void)
{
    ...
    switch (PolicySubIndex)
    {
        case 0:
            ...
            break;
        case 1:
            if ((isVBUSOverVoltage(VBUS_MDAC_4P62) || (ConnState == PoweredAccessory)) && (platform_check_timer(&SwapSourceStartTimer)))                    // Wait until we reach vSafe0V and delay if coming from PR Swap
            {
                PolicySubIndex++;
            }
            break;
        ...
        switch (PolicySubIndex)
        {
            case 0:
                if (PolicySendData(DMTSourceCapabilities, CapsHeaderSource.NumDataObjects, &CapsSource[0], peSourceSendCaps, 1, SOP_TYPE_SOP) == STAT_SUCCESS)
                {
                    ...
                    platform_set_timer(&PolicyStateTimer, tSenderResponse - tHardResetOverhead - 2);                     // Set the sender response timer
                }
                break;
            default:
                if (ProtocolMsgRx)                                              // If we have received a message
...
void PolicySourceTransitionSupply(void)
{
    ...
    switch (PolicySubIndex)
    {
        ...
        case 5:
            if(CapsSource[USBPDContract.FVRDO.ObjectPosition-1].FPDOSupply.Voltage == 100)
            {
                if((!isVBUSOverVoltage(VBUS_MDAC_5P04) && isVBUSOverVoltage(VBUS_MDAC_4P62)) || (platform_check_timer(&PolicyStateTimer)))   // Check that VBUS is between 4.6 and 5.5 V
                {
                    DetachThreshold = VBUS_MDAC_4P20;   // 5V detach threshold
                    PolicySubIndex++;
                }
            }
#ifdef FM150911A
...
void PolicySourceEvaluateDRSwap(void)
{
    ...
    else
    {
        ...
        if (Status == STAT_SUCCESS)                                             // If we received the good CRC...
        {
            ...
            Registers.Switches.DATAROLE = PolicyIsDFP;                          // Update the data role
            DeviceWrite(regSwitches1, 1, &Registers.Switches.byte[1]);          // Commit the data role in the 302 for the auto CRC
	   platform_set_data_role(PolicyIsDFP);
        }
    ...
    switch(PolicySubIndex)
    {
        ...
        case 1:
            if(platform_check_timer(&PolicyStateTimer))
            {
                platform_set_vbus_lvl_enable(VBUS_LVL_ALL, FALSE, FALSE);       // Disable VBUS output
                platform_set_vbus_discharge(TRUE);                              // Enabled VBUS discharge path
                platform_set_timer(&PolicyStateTimer, tPSHardResetMax + tSafe0V + 1000);                       // Timeout wait for vSafe0V
                PolicySubIndex++;
            }
            break;     
        case 2:
            ...
            {
                platform_set_timer(&PolicyStateTimer, tSrcTransition);                              // Allow some extra time for VBUS to discharge
                PolicyIsSource = FALSE;
                Registers.Switches.POWERROLE = PolicyIsSource;
                DeviceWrite(regSwitches1, 1, &Registers.Switches.byte[1]);
                PolicySubIndex++;
...
void PolicySourceEvaluateVCONNSwap(void)
{
    ...
    {
        case 0:
            if(!VCONN_Swap_To_Off_Supported)
            {
...
void PolicySinkDiscovery(void)
{
        ...
        PolicySubIndex = 0;
        platform_set_timer(&PolicyStateTimer, tTypeCSinkWaitCap);
		PolicySinkWaitCaps();
}
...
void PolicySinkWaitCaps(void)
{
    ...
    else if ((platform_check_timer(&PolicyStateTimer)) && (HardResetCounter <= nHardResetCount))
    {
    ...
    if (ProtocolMsgRx)
    {
        ...
        else                                                                    // Otherwise we received a data message...
        {
            switch (PolicyRxHeader.MessageType)                                 // Determine the message type
            {
                ...
                default:     
                                        PolicySubIndex = 0;
// If we receieved an unexpected data message...
                    PolicyState = peSinkSendHardReset;
                    break;
            }
        }
        ...
        default: // Allow time for the supply to rise and then send the PS_RDY message
            if (isVBUSOverVoltage(VBUS_MDAC_4P20))
            {
                ...
                if (Status == STAT_ERROR)
                    PolicyState = peErrorRecovery;                              // If we get an error, go to the error recovery state
                platform_set_timer(&SwapSourceStartTimer, tSwapSourceStart);
            }
            break;
    }
#endif // FSC_HAVE_DRP
...
void policyBISTCarrierMode2(void)
{
    switch (PolicySubIndex)
    {
        ...
        case 0:
            ...
            platform_set_timer(&PolicyStateTimer, tBISTContMode);                                        // Initialize and run BISTContModeTimer
            PolicySubIndex = 1;
            break;
        case 1:
            if(platform_check_timer(&PolicyStateTimer))                                                   // Transition to SRC_Transition_to_Default, SNK_Transition_to_Default, or CBL_Ready when BISTContModeTimer times out
            {
                Registers.Control.BIST_MODE2 = 0;                                           // Disable BIST_Carrier_Mode_2 (PD_RESET does not do this)
                DeviceWrite(regControl1, 1, &Registers.Control.byte[1]);
                platform_set_timer(&PolicyStateTimer, tGoodCRCDelay);                               // Delay to allow preamble to finish
                PolicySubIndex++;
            }
            break;
        case 2:
